...
class GameService:
    ...
    def create_game(
        ...
        existing_player = db.query(Player).filter(Player.session_id == session_id).first()
        
        if existing_player:
            # If player was host of another game, clear that
            if existing_player.game_id:
                old_game = db.query(Game).filter(Game.id == existing_player.game_id).first()
    ...
    @staticmethod
    def join_game(db: Session, code: str, player_name: str, session_id: str) -> Tuple[Optional[Game], Optional[Player]]:
    ...
    @staticmethod
    def reveal_player_card(db: Session, game_id: int, player_id: int, card_type: str) -> bool:
