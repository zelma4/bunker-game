...
from ..schemas import (
    GameCreate, GameResponse, GameJoin, PlayerResponse,
    VoteRequest, CharacterTraits, RevealCardRequest
)
...
async def join_game(
    ...
    game, player = GameService.join_game(db, join_data.code, join_data.player_name, session_id)
    ...
    for p in players:
        ...
        if not is_me and p.status == PlayerStatus.PLAYING:
            ...
            if "profession" not in revealed: p_resp.profession = None
            if "biology" not in revealed: p_resp.biology = None
            if "health" not in revealed: p_resp.health = None
            if "hobby" not in revealed: p_resp.hobby = None
            if "baggage" not in revealed: p_resp.baggage = None
            if "fact" not in revealed: p_resp.fact = None
...
async def start_game(
    ...
    if not host or host.session_id != session_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only host can start game")
...
async def toggle_ready(
    ...
    player.status = PlayerStatus.READY if player.status == PlayerStatus.WAITING else PlayerStatus.WAITING
...
async def reveal_card(
    ...
    success = GameService.reveal_player_card(db, game_id, player.id, card_data.card_type)
    
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot reveal this card"
        )
