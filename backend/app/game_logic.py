...
# Таблиця голосувань для кожного раунду
# Ключ = кількість гравців, значення = список голосувань для кожного раунду
# Гра закінчується коли залишається половина гравців (округлення вниз)
ROUNDS_TABLE = {
    4: [0, 1, 1],  # 4 гравці: 3 раунди, вигнання 2 осіб -> 2 виживають
    5: [0, 1, 1, 1],  # 5 гравців: 4 раунди, вигнання 3 осіб -> 2 виживають
    6: [0, 1, 1, 1],  # 6 гравців: 4 раунди, вигнання 3 осіб -> 3 виживають
    7: [0, 1, 1, 1, 1],  # 7 гравців: 5 раундів
...
def get_max_rounds(player_count: int) -> int:
    """
    Отримати максимальну кількість раундів для даної кількості гравців
    ...
    return len(ROUNDS_TABLE[player_count])
...
def get_votings_for_round(player_count: int, round_num: int) -> int:
    ...
    Args:
        player_count: кількість гравців
        round_num: номер раунду (1-based)
    ...
    max_rounds = get_max_rounds(player_count)
    if round_num < 1 or round_num > max_rounds:
        raise ValueError(
            f"Invalid round number: {round_num}. Must be between 1 and {max_rounds}"
        )
...
PHASE_DURATIONS = {
    ...
    "voting": 30,  # 30 сек на голосування
    "reveal": 3,  # 3 сек на показ результатів
